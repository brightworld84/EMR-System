...
export default function PostOpInfectionEducation() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get(`/patient-education-infection-risk/?checkin=${encodeURIComponent(checkinId)}`);
  ...
  const save = async (overrides = null) => {
    ...
    setSaving(true);
    try {
      const payload = { ...data, ...(overrides || {}), checkin: Number(checkinId) };
      let res;
      if (recordId) {
        res = await api.patch(`/patient-education-infection-risk/${recordId}/`, payload);
      } else {
        res = await api.post("/patient-education-infection-risk/", payload);
      }
      const saved = res.data;
      if (saved?.id) setRecordId(saved.id);
      setData({ ...defaults, ...saved, checkin: Number(checkinId) });
      if (!overrides?.is_complete) alert("Saved.");
      return saved;
    } catch (e) {
  ...
  const headerSubtitle = data.completed_at
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow print:hidden">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between gap-3 flex-wrap">
          <div>
            <h1 className="text-xl font-bold text-gray-900">Post-Op Infection Education</h1>
          ...
          <div className="flex gap-2 flex-wrap justify-end">
            ...
            <button onClick={() => window.print()} className="px-3 py-2 bg-gray-800 text-white rounded-lg hover:bg-black font-semibold">
              Print / PDF
            </button>
            <button
              onClick={() => save()}
            ...
            >
              {isLocked ? "Completed" : "Mark Complete"}
            </button>
          </div>
        </div>
      </header>
          ...
          <div className="bg-white rounded-lg shadow p-6 print:shadow-none print:p-0">
            ...
            <div className="space-y-6 text-sm text-gray-800 leading-relaxed">
              <section className="border border-gray-300 rounded-md p-4">
                ...
                <p className="mt-3">
                  You have probably heard the word <span className="font-bold">infection</span> before, but don't
                  really know the causes, signs, and symptoms and how you can prevent getting an infection.
                </p>
                <p className="mt-3">
                ...
                <p className="mt-3">
                  Unfortunately, surgery usually means you had an intentional break in the skin somewhere which puts
                  you at risk from the start, but there are many things you can do to help prevent an infection from
                  occurring:
                </p>
                <ul className="list-disc pl-6 mt-2 space-y-1">
                  <li>Demonstrate good hand washing technique.</li>
                  <li>Maintain good hygiene (shower daily unless otherwise specified by discharge instructions).</li>
                  <li>Keep dressing(s) clean and dry. If dressing(s) becomes soiled or wet before post-op visit, call your physician's office for recommendations.</li>
                  <li>Maintain a clean environment (regular dusting, sweeping, vacuuming, etc…).</li>
                  <li>If you are diabetic, keeping your sugars <span className="font-bold">UNDER CONTROL</span> will lessen your risk of getting an infection and promote healing.</li>
              ...
              <section className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                <div className="border border-gray-300 rounded-md p-4">
                  <TextLine label="Patient Acknowledgement Signature (type full name)" value={data.patient_ack_name} disabled={isLocked} onChange={(v) => setField("patient_ack_name", v)} />
                  <div className="mt-3">
                    <TextLine label="Date" value={data.patient_signature_date} disabled={isLocked} onChange={(v) => setField("patient_signature_date", v)} />
                  </div>
                </div>
                <div className="border border-gray-300 rounded-md p-4">
                  <TextLine label="Educating Nurse Signature (type full name)" value={data.educating_nurse_name} disabled={isLocked} onChange={(v) => setField("educating_nurse_name", v)} />
                  <div className="mt-3">
                    <TextLine label="Date" value={data.nurse_signature_date} disabled={isLocked} onChange={(v) => setField("nurse_signature_date", v)} />
                  </div>
                </div>
              </section>
            </div>
