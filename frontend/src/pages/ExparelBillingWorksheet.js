...
function ExparelBillingWorksheet() {
  ...
  const [data, setData] = useState({
    ...
    // TODO: add real worksheet fields to match paper
    notes: '',
  ...
  const loadOrCreate = async () => {
    ...
    try {
      const res = await api.get(`/exparel-billing-worksheet/?checkin=${encodeURIComponent(checkinId)}`);
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      const res = await api.patch(`/exparel-billing-worksheet/${formId}/`, payload);
  ...
  const signAndLock = async () => {
    ...
    try {
      ...
      const res = await api.post(`/exparel-billing-worksheet/${formId}/sign/`, {
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow print:hidden">
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col md:flex-row md:justify-between md:items-center gap-3">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Exparel Billing Worksheet</h1>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow p-6 space-y-6">
            <div className="text-sm text-gray-600">
              MVP scaffold: we’ll replace the fields to match the paper worksheet exactly.
            </div>
...
export default ExparelBillingWorksheet;
